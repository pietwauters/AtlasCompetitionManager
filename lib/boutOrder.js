...
// ---------------------------------------------------------------------------
// FIE Official Bout Orders — Individual Competitions
//
// STANDARD[6] history (see docs/format-system-comparison.md §7 for the full
// investigation): originally matched a "January 2004 Order of Bouts" document
// (docs/fie-order-of-bouts-2004.pdf, likely a USA Fencing domestic reference,
// not confirmed as FIE-international) that turned out to be the same table as
// the trio-of-3 special case (o.70.3.b). Corrected 2026-07-04 after real data
// from an actual FIE World Cup (Lion of Bonn 2019, Ophardt-tagged export)
// matched a *different* table exactly — one that also matches a column-major
// reading of the current FIE Organisation Rules PDF's plain o.69 pool-of-6
// table. Two independent real/current sources agreeing beat one document of
// uncertain/likely-domestic provenance, so STANDARD[6] now uses that table,
// and the old value moved to its own `TRIO_6` (still correct for the
// trio-conflict case — no evidence against it, and it has a plausible
// design reason: guaranteeing early trio fence-off).
//
// All other tables below (STANDARD 4-5, 7-12, PAIRS_6, TRIO_7, QUARTET_6)
// remain byte-verified against docs/fie-order-of-bouts-2004.pdf and/or real
// competition XML — see docs/format-system-comparison.md §7.
//
...
// Special bout order for pool of 6 when a TRIO of teammates is present.
// Trio placed at positions 1-2-3. Was aliased to STANDARD[6] until
// 2026-07-04 — that alias was correct back when STANDARD[6] held this same
// table; now that STANDARD[6] holds the (different) plain no-conflict table,
// TRIO_6 needs its own explicit value. See the header comment above.
const TRIO_6 = [[1,2],[4,5],[2,3],[5,6],[3,1],[6,4],[2,5],[1,4],[5,3],[1,6],[4,2],[3,6],[5,1],[3,4],[6,2]];
...
function selectBoutOrder(n, fencers, separation) {
  // Pools of 11 and 12: the FIE sequential tables have very poor rest
  // distribution (gaps of 1–10).  Use circle round-robin instead, which
  // gives uniform waves and much smaller gap variance.
  if (n === 11 || n === 12) {
    const { generateRoundRobinBoutList } = require('./multiStripPool');
    return generateRoundRobinBoutList(n);
  }
