...
function publishRecord(pisteId, pisteEntry, activeBoutId) {
  ...
  const payload = {
    ...
    referee:         Pipeline.refereeName(slot.referee_id),
    referee2:        officials.referee2,
    video_assistant: officials.video_assistant,
    assessor1:       officials.assessor1,
    assessor2:       officials.assessor2,
    bouts: bouts.map(b => {
...
function clearRecord(pisteId) {
  const msg = JSON.stringify({
    ...
    video_assistant: null, assessor1: null, assessor2: null, bouts: [],
  });
  rawPublish(topic(pisteId, 'software', 'record'), msg, { qos: 1, retain: true });
}
