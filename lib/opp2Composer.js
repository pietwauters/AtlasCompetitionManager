...
// The OPP2 wire protocol only ever has two phase_type values ('pool'/'DE') —
// this used to be a bare `type==='pool' ? 'pool' : 'DE'` ternary duplicated
// at three call sites, which silently treated anything else (including a
// virtual slot, services/pipelineVirtualSlots.js) as DE. Guards elsewhere
// (services/pipelineNav.js's activeSlot/prevSlotFor) should mean a virtual
// slot never reaches this file at all, but this is the defense-in-depth
// backstop if one ever does.
function phaseTypeForSlot(slot) {
  if (slot.type === 'pool') return 'pool';
  if (slot.type === 'de') return 'DE';
  return null;
}
...
function getBoutsForSlot(slot) {
  ...
  if (slot.type !== 'de') {
    throw new Error(`getBoutsForSlot: unsupported slot type "${slot.type}"`);
  }
...
// docs/level2.md §11/§19 — software/clock and software/uw2f seed a fresh
// apparatus session to its starting values (fresh bout or relay). Both are
// QoS 1, non-retained, via the standard envelope helper — same treatment as
// any other software-published correction (score, fencers, match).
function sendFreshClockAndUw2f(pisteId, pisteEntry) {
  publish(pisteId, 'clock', { ts: Date.now(), running: false, time_ms: 180000, time: '3:00' });
  publish(pisteId, 'uw2f',  { time_ms: 0, time: '0:00', right: { p_card: 0 }, left: { p_card: 0 } });
  if (pisteEntry) {
...
// docs/level2.md §11/§13/§19 piste-transfer use case — unlike
// sendFreshClockAndUw2f above (which always zeroes for a brand-new bout),
// this seeds a fresh apparatus on the new piste with the old apparatus's
// real last-known score/cards/priority/clock/UW2F, so an in-progress bout
// resumes instead of restarting from zero. No-op if no bout was active.
function seedTransferState(pisteId, pisteEntry) {
  if (!pisteEntry || !pisteEntry.boutId) return;

  if (pisteEntry.lastScore) {
    publish(pisteId, 'score', {
      left:     pisteEntry.lastScore.left,
      right:    pisteEntry.lastScore.right,
      priority: pisteEntry.lastScore.priority ?? 'N',
    });
  }

  const clock = pisteEntry.lastClock;
  publish(pisteId, 'clock', {
    ts: Date.now(), running: false,
    time_ms: clock?.time_ms ?? 180000,
    time:    clock?.time    ?? '3:00',
  });

  const uw2f = pisteEntry.lastUw2f;
  publish(pisteId, 'uw2f', {
    time_ms: uw2f?.time_ms ?? 0,
    time:    uw2f?.time    ?? '0:00',
    right:   uw2f?.right ?? { p_card: 0 },
    left:    uw2f?.left  ?? { p_card: 0 },
  });
}
...
function sendMatchData(pisteId, bout, slot, pisteEntry, { identifyingOnly = false } = {}) {
  ...
  publish(pisteId, 'match', {
    ...
    phase_type:  phaseTypeForSlot(slot),
  ...
  if (!identifyingOnly) {
    ...
    if (isFresh) sendFreshClockAndUw2f(pisteId, pisteEntry);
  }
}
