...
function _publishTeamMatchRecord(pisteId, pisteEntry, slot, activeBoutId) {
  ...
  const matchRow = db.prepare('SELECT phase_id FROM team_matches WHERE id = ?').get(slot.team_match_id);
  const phaseRow = matchRow ? db.prepare('SELECT rule_doc FROM phases WHERE id = ?').get(matchRow.phase_id) : null;
