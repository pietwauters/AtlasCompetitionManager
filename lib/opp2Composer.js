...
function sendMatchData(pisteId, bout, slot, pisteEntry) {
  ...
  if (slot.type === 'team_match') {
    return _sendRelayData(pisteId, bout, pisteEntry);
  }
...
function _sendRelayData(pisteId, relay, pisteEntry) {
  const fencers = {
  ...
  publish(pisteId, 'score', {
    left:  { score: relay.left_touches ?? 0, status: 'U', yellow_card: false, red_cards: 0, black_card: false },
    right: { score: relay.right_touches ?? 0, status: 'U', yellow_card: false, red_cards: 0, black_card: false },
    priority: 'N',
  }, { retain: true });
  ...
  if (pisteEntry) {
    pisteEntry.lastScore = isFresh
      ? null
      : { left: { score: relay.left_touches }, right: { score: relay.right_touches }, priority: 'N' };
  }
...
function _publishTeamMatchRecord(pisteId, pisteEntry, slot, activeBoutId) {
  const relays = db.prepare(`
    SELECT r.id, r.relay_number, r.left_touches, r.right_touches, r.status,
           r.left_position, r.right_position,
           COALESCE(sub_l.substitute_competitor_id, ord_l.competitor_id) AS left_comp_id,
           COALESCE(sub_r.substitute_competitor_id, ord_r.competitor_id) AS right_comp_id,
           lp.first_name AS left_first, lp.last_name AS left_last, lp.nationality AS left_nation,
           rp.first_name AS right_first, rp.last_name AS right_last, rp.nationality AS right_nation
    FROM relays r
    LEFT JOIN team_match_orders ord_l
      ON ord_l.team_match_id = r.team_match_id AND ord_l.position = r.left_position
    LEFT JOIN team_match_substitutions sub_l
      ON sub_l.team_match_id = r.team_match_id AND sub_l.team_id = ord_l.team_id
      AND sub_l.position_replaced = r.left_position AND sub_l.effective_from_relay <= r.relay_number
    LEFT JOIN team_match_orders ord_r
      ON ord_r.team_match_id = r.team_match_id AND ord_r.position = r.right_position
    LEFT JOIN team_match_substitutions sub_r
      ON sub_r.team_match_id = r.team_match_id AND sub_r.team_id = ord_r.team_id
      AND sub_r.position_replaced = r.right_position AND sub_r.effective_from_relay <= r.relay_number
    LEFT JOIN competitors lc ON lc.id = COALESCE(sub_l.substitute_competitor_id, ord_l.competitor_id)
    LEFT JOIN fencers lf ON lf.id = lc.fencer_id
    LEFT JOIN people  lp ON lp.id = lf.person_id
    LEFT JOIN competitors rc ON rc.id = COALESCE(sub_r.substitute_competitor_id, ord_r.competitor_id)
    LEFT JOIN fencers rf ON rf.id = rc.fencer_id
    LEFT JOIN people  rp ON rp.id = rf.person_id
    WHERE r.team_match_id = ?
    ORDER BY r.relay_number
  `).all(slot.team_match_id);
  ...
  const payload = {
    ...
    active_bout: effectiveActive,
    bouts: relays.map(r => {
      const ln = (r.left_first || r.left_last)  ? `${r.left_first || ''} ${r.left_last || ''}`.trim()  : null;
      const rn = (r.right_first || r.right_last) ? `${r.right_first || ''} ${r.right_last || ''}`.trim() : null;
      return {
        id:    String(r.id),
        order: r.relay_number,
        left:  r.left_comp_id  ? { id: String(r.left_comp_id),  name: ln, last_name: r.left_last  || '', first_name: r.left_first  || '', nation: r.left_nation  || '' } : null,
        right: r.right_comp_id ? { id: String(r.right_comp_id), name: rn, last_name: r.right_last || '', first_name: r.right_first || '', nation: r.right_nation || '' } : null,
