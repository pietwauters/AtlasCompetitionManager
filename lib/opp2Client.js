...
function sendMatchData(pisteId, bout, slot) {
  ...
  publish(pisteId, 'fencers', fencers, { retain: true });
  ...
  publish(pisteId, 'match', match, { retain: true });
...
function handleMessage(mqttTopic, rawMessage) {
  ...
  if (publisher === 'apparatus' && msgType === 'connection') {
    ...
    // Re-push the active match on reconnect so the apparatus gets its current
    // state immediately and doesn't need to send NEXT to find out.
    const s = pisteState[pisteId];
    if (online && s.boutId && s.lastBout && s.lastSlot) {
      const current = Bout.findById(s.boutId);
      if (current && current.status !== 'finished') {
        sendMatchData(pisteId, s.lastBout, s.lastSlot);
      }
    }
  }
