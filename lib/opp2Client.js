...
function handleMessage(mqttTopic, rawMessage) {
  ...
  // During normal operation boutId is already set so this block is skipped.
  if (publisher === 'software' && msgType === 'record') {
    const state = pisteState[pisteId];
    if (state && !state.boutId) {
      if (payload.active_bout) {
        ...
        if (bout && bout.status !== 'finished') {
          const slot = bout.pool_id
            ...
            : db.prepare('SELECT * FROM pipeline_slots WHERE phase_id = ? AND de_round = ?').get(bout.phase_id, bout.de_round);
          if (slot) {
            state.slotId      = slot.id;
            state.boutId      = boutId;
            state.lastBout    = bout;
          ...
          // Retained record points to a finished or missing bout — stale, clear it.
          console.log(`[OPP2] Clearing stale retained record for piste ${pisteId} (bout ${boutId})`);
          clearRecord(pisteId);
        }
      }
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      client.once('connect', () => {
        ...
        for (const pisteId of Object.keys(pisteState)) {
          publishConnection(pisteId, true);
          // Clear any stale retained software/fencers and software/match messages.
          // Per spec these topics must NOT be retained; an empty retained payload
          // is the standard MQTT way to delete a retained message from the broker.
          for (const t of ['fencers', 'match']) {
            client.publish(topic(pisteId, 'software', t), '', { qos: 1, retain: true });
          }
        }
        ...
        // Subscribe to own retained software/record to recover boutId after restart.
        client.subscribe('openpiste/+/software/record',       { qos: 1 });
