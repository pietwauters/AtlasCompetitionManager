...
// Fetch weapon + gender for a pipeline slot (joins up through pool/phase to competition).
...
function handleMessage(mqttTopic, rawMessage) {
  ...
  if (publisher === 'apparatus' && msgType === 'connection') {
    ...
    if (payload.online === true && !wasOnline) {
      ...
      if (state && state.lastBout && state.lastSlot) {
        ...
        try { sendMatchData(pisteId, state.lastBout, state.lastSlot, state); }
  ...
  if (publisher === 'apparatus' && msgType === 'state') {
    // React to apparatus entering Ending state. apparatus/state is retained, so
    // this fires on broker reconnect too — covering the case where the CMS was
    // offline when the apparatus/control END command arrived.
    // Guard: skip if the current bout is already finished (prevents double-processing
    // when apparatus/control END and the retained apparatus/state=E both arrive).
    if (payload.state === 'E') {
      const state = pisteState[pisteId];
      if (state && state.boutId) {
        const bout = Bout.findById(state.boutId);
        if (bout && bout.status !== 'finished') {
          console.log(`[OPP2] Piste ${pisteId} apparatus/state=E — evaluating END`);
