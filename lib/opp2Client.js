...
function handleNext(pisteId) {
  ...
  if (slot.id !== state.slotId) state.boutId = null;
...
function handleEnd(pisteId) {
  ...
  if (updatedBout?.pool_id)       SSE.emit(updatedBout.pool_id,          'bout-updated',    updatedBout);
  if (updatedBout?.phase_id)      SSE.emit(updatedBout.phase_id,         'bout-updated',    updatedBout);
  if (next?.phase_id)             SSE.emit(next.phase_id,                'bout-updated',    next);
  if (updatedBout?.competition_id) SSE.emit('comp_' + updatedBout.competition_id, 'results-updated', {});
