...
//   Main bracket goes only to lastMainRound = log2(fromTableau/reentryAt) + 1.
//   Repechage tables D/E/F/G (bracket='repechage') follow, then a fresh Finals
...
function buildFullBracket(competitors, rule) {
  ...
  const fromT        = isRepechage ? rule.repechage.fromTableau : null;
  const reT          = isRepechage ? rule.repechage.reentryAt   : null;
  const numRepRounds = isRepechage ? Math.round(Math.log2(fromT / reT)) : 0;
  const lastMainRound = isRepechage ? numRepRounds + 1 : null;

  // For repechage the tableau size is fixed by the rule; byes fill the remaining slots.
  const T = isRepechage ? fromT : getTableauSize(N);
  if (N > T) throw Object.assign(new Error(`Too many competitors (${N}) for this bracket size (${T}).`), { status: 400 });
